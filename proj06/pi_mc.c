...
int main(int argc, char** argv)
{
  ...
  int sd = atoi(argv[2]);
  int num_threads = 4; // Set to 4, but can change
  long i;  long Ncirc = 0;
  double pi, x, y, test;
  double r = 7.0;   // radius of circle. Side of squrare is 2*r

  // Set the number of OpenMP threads
  omp_set_num_threads(num_threads);
  
  // Beginning of Parallel region
  # pragma omp parallel private(x, y, test) reduction(+:Ncirc)
  {
    Seed(-r, r, sd + omp_get_num_threads());  // The circle and square are centered at the origin
    // omp for
    # pragma omp for
